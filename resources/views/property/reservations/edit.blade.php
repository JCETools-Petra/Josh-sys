<x-app-layout>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    {{-- Ganti action form ke rute 'property' --}}
                    <form action="{{ route('property.reservations.update', $reservation) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="room_type_id" id="room_type_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm" required>
                                    ...
                                    @foreach($roomTypes as $roomType)
                                        {{-- Pilih otomatis tipe kamar yang sudah ada di data reservasi --}}
                                        <option value="{{ $roomType->id }}" {{ old('room_type_id', $reservation->room_type_id) == $roomType->id ? 'selected' : '' }}>
                                            {{ $roomType->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="display_price" class="block mt-1 w-full bg-gray-100 dark:bg-gray-700" type="text" value="Rp {{ number_format($reservation->final_price, 0, ',', '.') }}" readonly />
                                <input type="hidden" name="final_price" id="final_price" value="{{ $reservation->final_price }}">
                            </div>
                            ...
                            <div class="col-span-2">
                                ...
                                <x-text-input id="guest_name" name="guest_name" :value="old('guest_name', $reservation->guest_name)" class="block mt-1 w-full" type="text" required />
                            </div>
                            
                            <div>
                                <x-input-label for="checkin_date" :value="__('Tanggal Check-in')" />
                                <x-text-input id="checkin_date" name="checkin_date" :value="old('checkin_date', $reservation->checkin_date->format('Y-m-d'))" class="block mt-1 w-full" type="date" required />
                            </div>

                            <div>
                                <x-input-label for="checkout_date" :value="__('Tanggal Check-out')" />
                                <x-text-input id="checkout_date" name="checkout_date" :value="old('checkout_date', $reservation->checkout_date->format('Y-m-d'))" class="block mt-1 w-full" type="date" required />
                            </div>

                             <div>
                                <x-input-label for="number_of_rooms" :value="__('Jumlah Kamar')" />
                                <x-text-input id="number_of_rooms" name="number_of_rooms" value="{{ old('number_of_rooms', $reservation->number_of_rooms) }}" class="block mt-1 w-full" type="number" required />
                            </div>
                        </div>
    ...
    @push('scripts')
    <script>
        // Logika JavaScript untuk mengupdate harga saat tipe kamar diubah
