        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('ecommerce.reservations.update', $reservation) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-2">
                                <x-input-label for="guest_name" :value="__('Nama Tamu')" />
                                <x-text-input id="guest_name" class="block mt-1 w-full" type="text" name="guest_name" :value="old('guest_name', $reservation->guest_name)" required autofocus />
                                <x-input-error :messages="$errors->get('guest_name')" class="mt-2" />
                            </div>

                            <div class="col-span-2">
                                <x-input-label for="guest_email" :value="__('Email Tamu (Opsional)')" />
                                <x-text-input id="guest_email" class="block mt-1 w-full" type="email" name="guest_email" :value="old('guest_email', $reservation->guest_email)" />
                                <x-input-error :messages="$errors->get('guest_email')" class="mt-2" />
                            </div>
                            
                            <div>
                                <x-input-label for="checkin_date" :value="__('Tanggal Check-in')" />
                                <x-text-input id="checkin_date" class="block mt-1 w-full" type="date" name="checkin_date" :value="old('checkin_date', $reservation->checkin_date->format('Y-m-d'))" required />
                                <x-input-error :messages="$errors->get('checkin_date')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="checkout_date" :value="__('Tanggal Check-out')" />
                                <x-text-input id="checkout_date" class="block mt-1 w-full" type="date" name="checkout_date" :value="old('checkout_date', $reservation->checkout_date->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="number_of_rooms" class="block mt-1 w-full" type="number" name="number_of_rooms" :value="old('number_of_rooms', $reservation->number_of_rooms)" required />
