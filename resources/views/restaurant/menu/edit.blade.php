...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="max-w-2xl bg-white rounded-lg shadow-lg p-6">
        <form method="POST" action="{{ route('restaurant.menu.update', $menuItem) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="category" id="category" required
                    ...
                    <option value="lunch" {{ old('category', $menuItem->category) === 'lunch' ? 'selected' : '' }}>🍱 Lunch</option>
                    <option value="dinner" {{ old('category', $menuItem->category) === 'dinner' ? 'selected' : '' }}>🍽️ Dinner</option>
                    <option value="appetizer" {{ old('category', $menuItem->category) === 'appetizer' ? 'selected' : '' }}>🥗 Appetizer</option>
                    <option value="main_course" {{ old('category', $menuItem->category) === 'main_course' ? 'selected' : '' }}>🍖 Main Course</option>
                    ...
                    <option value="snack" {{ old('category', $menuItem->category) === 'snack' ? 'selected' : '' }}>🍪 Snack</option>
                    <option value="alcohol" {{ old('category', $menuItem->category) === 'alcohol' ? 'selected' : '' }}>🍷 Alcohol</option>
                </select>
                @error('category')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $menuItem->price) }}" required min="0" step="1000"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500"
                       placeholder="25000">
                @error('price')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500"
                          placeholder="Brief description of the dish">{{ old('description', $menuItem->description) }}</textarea>
                @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
