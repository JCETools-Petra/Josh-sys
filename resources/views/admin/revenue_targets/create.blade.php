<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Tambah Target Pendapatan Baru') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('admin.revenue-targets.store') }}">
                        @csrf
                        ...
                        <div class="mt-4">
                            ...
                            <select id="property_id" name="property_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                ...
                                {{-- Pastikan variabel $properties dikirim dari controller --}}
                                @if(isset($properties) && !$properties->isEmpty())
                                    ...
                                    @endforeach
                                @else
                                    {{-- Opsi ini akan muncul jika $properties kosong atau tidak di-set,
                                         meskipun controller Anda seharusnya melakukan redirect jika kosong. --}}
                                    <option value="" disabled>Tidak ada properti tersedia. Silakan tambahkan properti terlebih dahulu.</option>
                                @endif
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="month_year" class="block mt-1 w-full" type="month" name="month_year" :value="old('month_year')" required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="target_amount" class="block mt-1 w-full" type="number" name="target_amount" :value="old('target_amount')" required step="1000" min="0" />
