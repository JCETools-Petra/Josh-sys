...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Edit Kamar Hotel</h1>
            <p class="text-gray-600">Edit informasi kamar {{ $hotelRoom->room_number }}</p>
        </div>
        <a href="{{ route('admin.hotel-rooms.index', ['property_id' => $hotelRoom->property_id]) }}"
    ...
    <form action="{{ route('admin.hotel-rooms.update', $hotelRoom) }}" method="POST" class="bg-white rounded-lg shadow-lg">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="property_id" id="property_id" required
                            ...
                            @foreach($properties as $property)
                                <option value="{{ $property->id }}" {{ old('property_id', $hotelRoom->property_id) == $property->id ? 'selected' : '' }}>
                                    {{ $property->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="room_number" required
                            value="{{ old('room_number', $hotelRoom->room_number) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="floor"
                            value="{{ old('floor', $hotelRoom->floor) }}"
            ...
            <div class="bg-green-50 border-l-4 border-green-500 p-4 rounded">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="room_type_id" id="room_type_id" required
                            ...
                            @foreach($roomTypes as $type)
                                <option value="{{ $type->id }}" {{ old('room_type_id', $hotelRoom->room_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="capacity" required min="1" value="{{ old('capacity', $hotelRoom->capacity) }}"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4 mt-3">
                            <label class="flex items-center">
                                <input type="radio" name="is_smoking" value="0" {{ old('is_smoking', $hotelRoom->is_smoking) == 0 ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-green-600 focus:ring-green-500">
                                <span class="ml-2 text-sm">🚭 Non-Smoking</span>
                            </label>
                            <label class="flex items-center">
                                <input type="radio" name="is_smoking" value="1" {{ old('is_smoking', $hotelRoom->is_smoking) == 1 ? 'checked' : '' }}
            ...
            <div class="bg-purple-50 border-l-4 border-purple-500 p-4 rounded">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="status" required
                            ...
                            <option value="vacant_clean" {{ old('status', $hotelRoom->status) == 'vacant_clean' ? 'selected' : '' }}>Siap (Vacant Clean)</option>
                            <option value="vacant_dirty" {{ old('status', $hotelRoom->status) == 'vacant_dirty' ? 'selected' : '' }}>Kotor (Vacant Dirty)</option>
                            <option value="occupied" {{ old('status', $hotelRoom->status) == 'occupied' ? 'selected' : '' }}>Terisi (Occupied)</option>
                            <option value="maintenance" {{ old('status', $hotelRoom->status) == 'maintenance' ? 'selected' : '' }}>Perbaikan (Maintenance)</option>
                            <option value="out_of_order" {{ old('status', $hotelRoom->status) == 'out_of_order' ? 'selected' : '' }}>Rusak (Out of Order)</option>
                            <option value="blocked" {{ old('status', $hotelRoom->status) == 'blocked' ? 'selected' : '' }}>Diblokir (Blocked)</option>
                        </select>
                        @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="assigned_hk_user_id"
                            ...
                            @foreach($housekeepers as $hk)
                                <option value="{{ $hk->id }}" {{ old('assigned_hk_user_id', $hotelRoom->assigned_hk_user_id) == $hk->id ? 'selected' : '' }}>
                                    {{ $hk->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                @if($hotelRoom->last_cleaned_at)
                <div class="mt-4 bg-white p-3 rounded border border-purple-200">
                    <p class="text-sm text-gray-600">
                        <strong>Terakhir dibersihkan:</strong> {{ $hotelRoom->last_cleaned_at->format('d M Y H:i') }}
                        ({{ $hotelRoom->last_cleaned_at->diffForHumans() }})
                    </p>
                </div>
                @endif
            </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                            @php
                                $existingFeatures = old('features', $hotelRoom->features ?? []);
                            @endphp
                            <label class="flex items-center">
                                <input type="checkbox" name="features[]" value="WiFi"
                                    {{ in_array('WiFi', $existingFeatures) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-yellow-600">
                                <span class="ml-2 text-sm">WiFi</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="features[]" value="TV"
                                    {{ in_array('TV', $existingFeatures) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-yellow-600">
                                <span class="ml-2 text-sm">TV</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="features[]" value="AC"
                                    {{ in_array('AC', $existingFeatures) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-yellow-600">
                                <span class="ml-2 text-sm">AC</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="features[]" value="Minibar"
                                    {{ in_array('Minibar', $existingFeatures) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-yellow-600">
                                <span class="ml-2 text-sm">Minibar</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="features[]" value="Balcony"
                                    {{ in_array('Balcony', $existingFeatures) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-yellow-600">
                                <span class="ml-2 text-sm">Balkon</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="features[]" value="Bathtub"
                                    {{ in_array('Bathtub', $existingFeatures) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-yellow-600">
                                <span class="ml-2 text-sm">Bathtub</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="features[]" value="Safe Box"
                                    {{ in_array('Safe Box', $existingFeatures) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-yellow-600">
                                <span class="ml-2 text-sm">Safe Box</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="features[]" value="Hair Dryer"
                                    {{ in_array('Hair Dryer', $existingFeatures) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-yellow-600">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="3"
                            ...
                            placeholder="Catatan tambahan tentang kamar ini...">{{ old('notes', $hotelRoom->notes) }}</textarea>
                        @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentRoomTypeId = {{ $hotelRoom->room_type_id }};
