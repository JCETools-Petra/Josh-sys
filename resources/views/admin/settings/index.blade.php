                            ...
                            <div class="mt-4">
                                ...
                                <div class="mt-2 flex items-center space-x-4">
                                    @if(isset($settings['logo_path']) && $settings['logo_path']->value)
                                        <div class="relative">
                                            <img src="{{ asset('storage/' . $settings['logo_path']->value) }}?v={{ time() }}" alt="Logo saat ini" class="h-16 w-16 bg-gray-200 dark:bg-gray-700 p-1 rounded-md object-contain">
                                            <div class="absolute -top-2 -right-2 bg-green-500 text-white text-xs px-2 py-1 rounded-full">
                                                Current
                                            </div>
                                        </div>
                                    @else
                                        <div class="h-16 w-16 bg-gray-200 dark:bg-gray-700 p-1 rounded-md flex items-center justify-center">
                                            <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                            </svg>
                                        </div>
                                    @endif
                                    <x-text-input id="logo_path" name="logo_path" type="file" accept="image/png,image/jpg,image/jpeg,image/gif" class="block w-full" />
                                </div>
                                <p class="text-xs text-gray-500 mt-1">Format yang didukung: PNG, JPG, JPEG, GIF (Max: 2MB)</p>
                            ...
                            <div class="mt-4">
                                ...
                                <div class="mt-2 flex items-center space-x-4">
                                    @if(isset($settings['favicon_path']) && $settings['favicon_path']->value)
                                        <div class="relative">
                                            <img src="{{ asset('storage/' . $settings['favicon_path']->value) }}?v={{ time() }}" alt="Favicon saat ini" class="h-8 w-8 bg-gray-200 dark:bg-gray-700 p-1 rounded-md object-contain">
                                            <div class="absolute -top-2 -right-2 bg-green-500 text-white text-xs px-1 py-0.5 rounded-full">
                                                ✓
                                            </div>
                                        </div>
                                    @else
                                        <div class="h-8 w-8 bg-gray-200 dark:bg-gray-700 p-1 rounded-md flex items-center justify-center">
                                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                            </svg>
                                        </div>
                                    @endif
                                    <x-text-input id="favicon_path" name="favicon_path" type="file" accept="image/png,image/x-icon" class="block w-full" />
                                </div>
                                <p class="text-xs text-gray-500 mt-1">Unggah gambar kotak (misal: 32x32 atau 64x64 piksel) untuk hasil terbaik. Format: PNG atau ICO (Max: 512KB)</p>
