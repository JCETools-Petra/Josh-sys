...
class BookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $eventDate = \Carbon\Carbon::parse($validatedData['event_date'])->format('d M Y');
        $roomName = \App\Models\Room::find($validatedData['room_id'])->name ?? 'N/A';
        $categoryName = $validatedData['mice_category_id'] ? (\App\Models\MiceCategory::find($validatedData['mice_category_id'])->name ?? 'N/A') : 'N/A';

        \App\Models\ActivityLog::create([
            'user_id' => $user->id,
            'property_id' => $user->property_id,
            'action' => 'create',
            'description' => $user->name . " membuat booking MICE baru #{$booking->booking_number} untuk {$validatedData['client_name']}, event: {$eventDate}, ruangan: {$roomName}, kategori: {$categoryName}, pax: {$validatedData['participants']}, total: Rp " . number_format($validatedData['total_price'], 0, ',', '.'),
            'loggable_id' => $booking->id,
            'loggable_type' => \App\Models\Booking::class,
    ...
    public function update(Request $request, Booking $booking)
    {
        ...
        $oldClient = $booking->client_name;
        $oldEventDate = $booking->event_date->format('d M Y');
        ...
        // Log activity dengan detail lengkap
        $newEventDate = \Carbon\Carbon::parse($validatedData['event_date'])->format('d M Y');
        $changes = [];
        if ($oldClient !== $validatedData['client_name']) $changes[] = "client: {$oldClient} → {$validatedData['client_name']}";
        if ($oldEventDate !== $newEventDate) $changes[] = "tanggal: {$oldEventDate} → {$newEventDate}";
        if ($oldStatus !== $validatedData['status']) $changes[] = "status: {$oldStatus} → {$validatedData['status']}";
        if ($oldPrice != $validatedData['total_price']) $changes[] = "harga: Rp " . number_format($oldPrice, 0, ',', '.') . " → Rp " . number_format($validatedData['total_price'], 0, ',', '.');

        $changeDetails = !empty($changes) ? implode(', ', $changes) : 'data detail';

        \App\Models\ActivityLog::create([
            'user_id' => Auth::id(),
            'property_id' => $booking->property_id,
            'action' => 'update',
            'description' => Auth::user()->name . " mengubah booking MICE #{$booking->booking_number}: {$changeDetails}",
            'loggable_id' => $booking->id,
            'loggable_type' => \App\Models\Booking::class,
            'ip_address' => $request->ip(),
            'user_agent' => $request->userAgent(),
    ...
    public function destroy(Booking $booking)
    {
        ...
        \App\Models\ActivityLog::create([
            ...
            'action' => 'delete',
            'description' => Auth::user()->name . " menghapus booking MICE #{$bookingNumber} untuk {$clientName}, event: {$eventDate}, total: Rp " . number_format($totalPrice, 0, ',', '.'),
    ...
    public function storeBeo(Request $request, Booking $booking)
    {
        ...
        // ✅ STABILITY FIX: Validate price package exists before proceeding
        if (!$pricePackage) {
            return redirect()->back()->with('error', 'Price package tidak ditemukan.');
        }
        ...
        // Log activity dengan detail lengkap
        $functionSheet = $booking->functionSheet;
        \App\Models\ActivityLog::create([
            'user_id' => auth()->id(),
            'property_id' => $booking->property_id,
            'action' => $functionSheet->wasRecentlyCreated ? 'create' : 'update',
            'description' => auth()->user()->name . " " . ($functionSheet->wasRecentlyCreated ? 'membuat' : 'mengubah') . " BEO #{$functionSheet->beo_number} untuk booking {$booking->client_name}, event: {$booking->event_date->format('d M Y')}, setup: {$validated['room_setup']}, paket: {$pricePackage->name}, pax: {$booking->participants}, total: Rp " . number_format($totalPrice, 0, ',', '.'),
            'loggable_id' => $booking->id,
            'loggable_type' => \App\Models\Booking::class,
            'ip_address' => $request->ip(),
            'user_agent' => $request->userAgent(),
    ...
    public function printBeo(Booking $booking)
    {
        ...
        \App\Models\ActivityLog::create([
            ...
            'property_id' => $booking->property_id,
            'action' => 'view',
            'description' => auth()->user()->name . " mencetak BEO #{$booking->functionSheet->beo_number} untuk booking {$booking->client_name}, event: {$booking->event_date->format('d M Y')}",
            'loggable_id' => $booking->id,
            'loggable_type' => \App\Models\Booking::class,
            'ip_address' => request()->ip(),
            'user_agent' => request()->userAgent(),
    ...
    public function downloadBeo(Booking $booking)
    {
        if (!$booking->functionSheet || auth()->user()->property_id != $booking->property_id) {
            return redirect()->back()->with('error', 'Aksi tidak diizinkan.');
        }

        $pdf = \PDF::loadView('sales.bookings.print-beo', [
            'booking' => $booking,
            'beo' => $booking->functionSheet
        ]);

        $filename = 'BEO_' . $booking->booking_number . '_' . now()->format('Ymd') . '.pdf';
        ...
        \App\Models\ActivityLog::create([
            ...
            'property_id' => $booking->property_id,
            'action' => 'export',
            'description' => auth()->user()->name . " mengunduh BEO PDF #{$booking->functionSheet->beo_number} untuk booking {$booking->client_name}, event: {$booking->event_date->format('d M Y')}, file: {$filename}",
            'loggable_id' => $booking->id,
            'loggable_type' => \App\Models\Booking::class,
            'ip_address' => request()->ip(),
            'user_agent' => request()->userAgent(),
