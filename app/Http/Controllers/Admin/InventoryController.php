...
class InventoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        do {
            $itemCode = $category->category_code . '-' . strtoupper(\Illuminate\Support\Str::random(6));
        } while (Inventory::where('item_code', $itemCode)->exists());
    ...
    {
        $query = HkAssignment::query()
            ->join('inventories', 'hk_assignments.inventory_id', '=', 'inventories.id')
            ...
            ->where('categories.name', 'ROOM AMENITIES')
            ->select(
                'properties.name as property_name',
                'inventories.name as amenity_name',
                DB::raw('SUM(hk_assignments.quantity_used) as total_used')
            )
            ->groupBy('properties.id', 'properties.name', 'inventories.id', 'inventories.name');

        // Filter berdasarkan tanggal jika ada
        if ($request->filled('start_date')) {
            $query->whereDate('hk_assignments.created_at', '>=', $request->start_date);
        }

        if ($request->filled('end_date')) {
            $query->whereDate('hk_assignments.created_at', '<=', $request->end_date);
        }

        $reportData = $query->orderBy('property_name')
