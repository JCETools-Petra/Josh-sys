...
class BudgetDebugController extends Controller
{
    ...
    public function verify(Request $request, Property $property)
    {
        ...
        // Eager load all categories to prevent N+1 queries
        $categoryIds = $grouped->keys()->toArray();
        $categories = FinancialCategory::whereIn('id', $categoryIds)->get()->keyBy('id');
    ...
    public function api(Request $request, Property $property)
    {
        ...
        // ⚡ PERFORMANCE FIX: Eager load all categories before loop to prevent N+1
        $categoryIds = $grouped->keys()->toArray();
        $categories = FinancialCategory::whereIn('id', $categoryIds)->get()->keyBy('id');

        $results = [];
        foreach ($grouped as $catId => $catEntries) {
            // ⚡ Use pre-loaded category instead of querying in loop
            $category = $categories->get($catId);
            $results[] = [
                'category_id' => $catId,
