...
// Front Office Routes - Accessible by: pengguna_properti, owner, admin
// 🔧 BUG FIX: Add role middleware to ensure property users can access front office
Route::middleware(['auth', 'verified', 'role:pengguna_properti,owner,admin'])->prefix('frontoffice')->name('frontoffice.')->group(function () {
    ...
    Route::get('/booking-history', [\App\Http\Controllers\FrontOfficeController::class, 'bookingHistory'])->name('booking-history');
    ...
    // 🔧 BUG FIX: Add route to verify/confirm a reservation for check-in
    Route::post('/verify-checkin/{roomStay}', [\App\Http\Controllers\FrontOfficeController::class, 'verifyCheckIn'])->name('verify-checkin');
    ...
    // 💰 Payment Management
    Route::post('/folio/{roomStay}/add-payment', [\App\Http\Controllers\FolioController::class, 'addPayment'])->name('folio.add-payment');
    Route::put('/folio/{roomStay}/payment/{payment}', [\App\Http\Controllers\FolioController::class, 'updatePayment'])->name('folio.update-payment');
    Route::delete('/folio/{roomStay}/payment/{payment}', [\App\Http\Controllers\FolioController::class, 'deletePayment'])->name('folio.delete-payment');

    // No-Show Management
    Route::get('/no-show/{roomStay}', [\App\Http\Controllers\FrontOfficeController::class, 'markNoShow'])->name('no-show.show');
    Route::post('/no-show/{roomStay}', [\App\Http\Controllers\FrontOfficeController::class, 'processNoShow'])->name('no-show.process');

    // Early Check-in / Late Checkout
    Route::post('/early-checkin/{roomStay}', [\App\Http\Controllers\FrontOfficeController::class, 'requestEarlyCheckin'])->name('early-checkin.request');
    Route::post('/late-checkout/{roomStay}', [\App\Http\Controllers\FrontOfficeController::class, 'requestLateCheckout'])->name('late-checkout.request');

    // Cash Drawer Management
    Route::get('/cash-drawer', [\App\Http\Controllers\Admin\CashDrawerController::class, 'index'])->name('cash-drawer.index');
    Route::get('/cash-drawer/create', [\App\Http\Controllers\Admin\CashDrawerController::class, 'create'])->name('cash-drawer.create');
    Route::post('/cash-drawer', [\App\Http\Controllers\Admin\CashDrawerController::class, 'store'])->name('cash-drawer.store');
    Route::get('/cash-drawer/{id}', [\App\Http\Controllers\Admin\CashDrawerController::class, 'show'])->name('cash-drawer.show');
    Route::get('/cash-drawer/{id}/close', [\App\Http\Controllers\Admin\CashDrawerController::class, 'edit'])->name('cash-drawer.edit');
    Route::post('/cash-drawer/{id}/close', [\App\Http\Controllers\Admin\CashDrawerController::class, 'close'])->name('cash-drawer.close');
    Route::post('/cash-drawer/{id}/transaction', [\App\Http\Controllers\Admin\CashDrawerController::class, 'addTransaction'])->name('cash-drawer.transaction');
    Route::get('/cash-drawer-history', [\App\Http\Controllers\Admin\CashDrawerController::class, 'history'])->name('cash-drawer.history');
});
...
Route::middleware(['auth', 'verified'])->prefix('restaurant')->name('restaurant.')->group(function () {
    ...
    Route::get('/orders/history', [\App\Http\Controllers\RestaurantController::class, 'orderHistory'])->name('orders.history');
    Route::get('/orders/{order}', [\App\Http\Controllers\RestaurantController::class, 'orderDetail'])->name('orders.detail');
    ...
    // Sales report
    Route::get('/sales-report', [\App\Http\Controllers\RestaurantController::class, 'salesReport'])->name('sales-report');

    // Guest lookup by room number (API for POS)
    Route::get('/api/guest-by-room/{roomNumber}', [\App\Http\Controllers\RestaurantController::class, 'guestByRoom'])->name('api.guest-by-room');
...
// Housekeeping Routes - Role: hk, pengguna_properti, owner (Staff Housekeeping & Property Users)
Route::middleware(['auth', 'verified', 'role:hk,pengguna_properti,owner'])->prefix('housekeeping')->name('housekeeping.')->group(function () {
    ...
    Route::post('/assign-room/{room}', [\App\Http\Controllers\HousekeepingDashboardController::class, 'assignRoom'])->name('assign-room');
    ...
    Route::get('/lost-found/{lostAndFound}/claim', [\App\Http\Controllers\LostAndFoundController::class, 'showClaimForm'])->name('lost-found.claim-form');
...
Route::middleware(['auth', 'verified'])->prefix('maintenance')->name('maintenance.')->group(function () {
    ...
    Route::patch('/{maintenanceRequest}', [\App\Http\Controllers\MaintenanceRequestController::class, 'update'])->name('update');
    Route::delete('/{maintenanceRequest}', [\App\Http\Controllers\MaintenanceRequestController::class, 'destroy'])->name('destroy');
});
...
Route::middleware(['auth', 'verified'])->prefix('refunds')->name('refunds.')->group(function () {
    ...
    Route::post('/{refund}/process', [\App\Http\Controllers\RefundController::class, 'process'])->name('process');
...
Route::prefix('admin')->middleware(['auth', 'verified', 'role:admin,owner'])->name('admin.')->group(function () {
    ...
    Route::get('/activity-log/export', [ActivityLogController::class, 'export'])->name('activity_log.export');
...
Route::prefix('property')->middleware(['auth', 'verified', 'role:pengguna_properti,owner,sales'])->name('property.')->group(function () {
