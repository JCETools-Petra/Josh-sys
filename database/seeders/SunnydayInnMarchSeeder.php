...
class SunnydayInnMarchSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $totals = [
            'corp_income'         => 12_789_257,  // Corporate
            'offline_room_income' => 60_723_140,  // Walk-in
            'ta_income'           => 25_324_115,  // OTA/TA
            'breakfast_income'    => 28_198_207,  // Breakfast
            'lunch_income'        => 8_092_562,   // Restaurant (+ room service 0)
            'others_income'       => 23_243_802,  // Other + Wellness + Laundry
        ];
        $targetNonMice = array_sum($totals); // 158,371,083

        $this->command->info("→ Menulis NON-MICE ke daily_incomes (Mar 2025), target: " . number_format($targetNonMice, 0, ',', '.'));

        // Bersihkan Maret
        DB::table('daily_incomes')
            ->where('property_id', $propertyId)
            ->whereBetween('date', ['2025-03-01', '2025-03-31'])
            ->delete();
        ...
        for ($d = 1; $d <= $days; $d++) {
            $date = Carbon::create(2025, 3, $d)->toDateString();
            $i    = $d - 1;
            ...
            DB::table('daily_incomes')->insert([
                ...
                'arr'                  => number_format(rand(300000, 420000), 2, '.', ''),
                'occupancy'            => number_format(rand(20, 65), 2, '.', ''),
                'created_at'           => now(),
                'updated_at'           => now(),
            ]);
        }

        // ===== MICE → bookings (total 131,157,025) =====
        $this->command->info("→ Menulis MICE ke bookings (Mar 2025)...");
        DB::table('bookings')
            ->where('property_id', $propertyId)
            ->whereBetween('event_date', ['2025-03-01', '2025-03-31'])
            ->delete();

        $bookings = [
            [
                'client_name'   => 'PT. Sawit Merauke',
                'event_type'    => 'Rapat Evaluasi Triwulan',
                'event_date'    => '2025-03-06',
                'participants'  => 120,
                'total_price'   => 35_500_000,
                'notes'         => 'Full-day + coffee break.',
            ],
            [
                'client_name'   => 'Dinas Perhubungan',
                'event_type'    => 'Sosialisasi Keselamatan',
                'event_date'    => '2025-03-13',
                'participants'  => 90,
                'total_price'   => 28_657_025,
                'notes'         => 'Include LCD projector & backdrop.',
            ],
            [
                'client_name'   => 'Komunitas UMKM Merauke',
                'event_type'    => 'Expo & Talkshow',
                'event_date'    => '2025-03-20',
                'participants'  => 200,
                'total_price'   => 42_000_000,
                'notes'         => 'Hall + sound system.',
            ],
            [
                'client_name'   => 'PT. Kargo Timur',
        ...
        foreach ($bookings as $i => $b) {
            DB::table('bookings')->insert([
                'booking_number'   => sprintf('MICE-202503-%04d', $i + 1),
